...
int main(void)
{
    ...
    // repeat the above tests with different insertion orders
    printf("\nRepeat unsorted insert tests with a different insertion order.\n");
    llist_t *order_test_list = llist_construct(ids_compare_genid);

    datap = (data_t *)calloc(1, sizeof(data_t));
    datap->generator_id = 100;
    llist_insert(order_test_list, datap, LLPOSITION_BACK);

    datap = (data_t *)calloc(1, sizeof(data_t));
    datap->generator_id = 200;
    llist_insert(order_test_list, datap, LLPOSITION_FRONT);

    datap = (data_t *)calloc(1, sizeof(data_t));
    datap->generator_id = 990;
    llist_insert(order_test_list, datap, 1);

    datap = llist_access(order_test_list, 0);
    if (datap != NULL)
    {
        printf("Order test: position 0 should be 200 and is %d\n", datap->generator_id);
        datap = llist_access(order_test_list, 1);
        printf("Order test: position 1 should be 999 and is %d\n", datap->generator_id);
        datap = llist_access(order_test_list, LLPOSITION_BACK);
        printf("Order test: position 2 should be 100 and is %d\n", datap->generator_id);
    }
    else
    {
        printf("ERROR: different-order insert test failed\n");
    }

    llist_destruct(order_test_list);
    ...
    // *************************************************************/
    printf("\n\nTest the remove function on a small unsorted list\n");
    llist_t *remove_list = llist_construct(ids_compare_genid);
    for (int id = 10; id <= 40; id += 10)
    {
        datap = (data_t *)calloc(1, sizeof(data_t));
        datap->generator_id = id;
        llist_insert(remove_list, datap, LLPOSITION_BACK);
    }
    llist_debug_print(remove_list);

    data_t *removed = llist_remove(remove_list, LLPOSITION_FRONT);
    if (removed != NULL)
    {
        printf("Removed front item with generator ID %d\n", removed->generator_id);
        free(removed);
    }
    llist_debug_print(remove_list);

    removed = llist_remove(remove_list, 1);
    if (removed != NULL)
    {
        printf("Removed middle item with generator ID %d\n", removed->generator_id);
        free(removed);
    }
    llist_debug_print(remove_list);

    removed = llist_remove(remove_list, LLPOSITION_BACK);
    if (removed != NULL)
    {
        printf("Removed tail item with generator ID %d\n", removed->generator_id);
        free(removed);
    }
    llist_debug_print(remove_list);

    removed = llist_remove(remove_list, 99);
    if (removed == NULL)
    {
        printf("Invalid remove index correctly returned NULL\n");
    }
    else
    {
        printf("ERROR: invalid remove index did not return NULL\n");
        free(removed);
    }

    // when done with testing clean up
    llist_destruct(remove_list);
